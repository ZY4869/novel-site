...
const CHAPTER_REGEX = /^\s*(?:第\s*([0-9一二三四五六七八九十百千]+)\s*[章节回卷集部幕]|chapter\s+\d+|CHAPTER\s+\d+)\s*(.{0,60})$/i;
...
async function onFileChange() {
  ...
  try {
    ...
    parsedChapters = splitChapters(text) || splitBySize(text, 6000);
    renderPreview();
    showMsg('import-msg', '', '');
    toggleTxtImportMode();
  } catch (e) {
...
function splitBySize(text, size = 6000) {
  const chapters = [];
